...
- [x] 전국 응급의료기관 목록 (병원명·등급·주소·전화) — 실시간 혼잡도가 없는 지역의 안전망으로 유지 (`src/er_directory.py`, `getEgytListInfoInqire`)
- [x] 공휴일(QT=8) 진료 병의원 전국 조회 + "의원"으로만 표시되던 곳의 진료과목 표시 (`src/duty_data.py`, `src/collect_duty.py`)
- [x] **전국 17개 지역 실시간 응급실 혼잡도** — `STAGE1=시도`만으로 지역 전체가 한 번에 조회됨 (지역당 API 1번, 전국 합계 약 444곳, `src/er_congestion.py`). 목록은 접이식(기본: 상태·응급실 병상만, 누르면 입원실·중환자실·수술실·갱신시각 펼침), 병상 수는 그리드 고정 열이라 병원명 길이와 상관없이 일렬 정렬
